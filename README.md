...
**简介**：
datawork是一个本地知识库工具，支持本地markdown全文检索与提取，支持多选项卡记纯文本笔记，支持国内外AI大模型调用，支持ollama和部分第三方大模型api供应商，旨在帮助个人更好地用上自己的知识和通过AI调用知识。项目基于python的tkinter构建，目前已支持windows。截至2025年7月，datawork中的agent已拥有多轮MCP工具调用和代码执行能力，个人可通过datawork打造专属的本地agent系统。


datawork v3.0 for windows 已发布：【全新支持本地向量知识库】【支持直接管理python环境和运行python代码】【支持搭建个性化ai数据处理流水线】【支持本地mcp】【支持离线语音识别】【支持手机访问ollama、agent和mcp】【快速将一个python脚本变成MCP工具】【最近更新时间：2025.7.2】
...
为方便个人和企业能把握时间窗口，更好地探索AI、用上AI，自2025年1月22日起，datawork的向量知识库功能支持在2025年全年免费试用，支持个人使用，也支持在公司中整个团队使用。

使用方法：进入agents界面的向量知识库，在授权区填写下方的 key 即可。

试用时间：2025.1.22—2026.2.1

试用 key ：datawork-vector-b5f2683b-940-20260201
...
- AI：对话。支持结构化提示词，可自定义系统级prompt；支持精准控制多轮会话准确度；可通过日志查看功能调用链及响应；自定义agent；多agent对话；多agent自动主题讨论；本地mcp；离线语音识别；agent多轮MCP工具调用；自定义MCP开发。
...
- 通过本地向量知识库探索个人简历，探索专业文档和电子书。
- 通过本地向量知识库进行“专业”知识学习，如代码研读。
- 运行python代码；python虚拟环境管理、版本选择、包库管理一站式解决。
- 通过简单代码快速搭建ai自动化流水线，实现自动翻译、自动阅读。
- 通过mcp访问本地文件和数据库。
- 离线语音识别（语音输入、录音、音频转写）。
- 将datawork作为服务器，与手机连通，支持手机访问ollama、agent和mcp。
- 让agent执行python代码和虚拟环境管理，结合MCP工具多轮调用，完成复杂任务。
- 将一个python脚本变成自定义MCP工具，将datawork打造为个人专属的agent系统。
- 更多场景...
...
- 1）MIT LICENSE 不对 datawork app 起作用，保留一切权利；
- 2）提示：目前软件处于快速更新阶段，使用时应注意个人数据备份，使用时责任自负。
