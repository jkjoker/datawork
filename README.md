...
datawork v3.0 for windows 已发布：【全新支持本地向量知识库】【支持直接管理python环境和运行python代码】【支持搭建个性化ai数据处理流水线】【支持本地mcp】【支持离线语音识别】【支持手机访问ollama、agent和mcp】【最近更新时间：2025.6.2】
...
- 通过mcp访问本地文件和数据库。
- 离线语音识别（语音输入、录音、音频转写）。
- datawork作为服务器，与手机连通，支持手机访问ollama、agent和mcp。
- 更多场景...
...
- AI：对话。支持结构化提示词，可自定义系统级prompt；支持精准控制多轮会话准确度；可通过日志查看功能调用链及响应；自定义agent；多agent对话；多agent自动主题讨论；本地mcp；离线语音识别。
